  ...
  return (
    <>
      <SEO
        title={
          profileData
            ? `${profileData.user?.name} | ${profileData.category?.name}`
            : "Professional Profile | HunarHub"
        }
        description={
          profileData?.bio ?? "Book trusted local professionals on HunarHub."
        }
        url={`https://hunar-hub-web.vercel.app/profile/${id}`}
      />
              ...
              <div className="space-y-6 max-md:space-y-3">
                ...
                >
                  ...
                  <div className="text-xs bg-green-50 text-green-700 border border-green-200 rounded-lg px-3 py-2 flex items-start gap-2">
                    <span>🛡️</span>
                    <span>
                      Book through our platform for{" "}
                      <b>verified professionals</b> and reliable support.
                    </span>
                  </div>
